...
mathematician is encouaged to attempt to solve this problem using
...
Hence our volume is
\[
\d V = 2\pi y (\answer[given]{\frac{y+1}{2} -y^2}) \d y
\]
Thus the volume is
\[
\textrm{Volume} = \int_{\answer[given]{0}}^{\answer[given]{1}} \answer{2\pi y (\answer[given]{\frac{y+1}{2} - y^2})} \d y
\]
\end{explanation}
\end{example}
