...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{align*}
      \int_{0}^{1} \answer[given]{-0.98} \d x &= \eval{\answer[given]{-0.98 s}}_0^1\\
      &=\answer[given]{-0.98}.
    \end{align*}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \[
    60\times 38.808 = 2,328.48 \unit{J}.
    \]
...
\begin{example}
  ...
  \begin{explanation}
    ...
    distance of $\answer[given]{30-y +5}$, so the total work done by
    moving each slab is
          \[
...
\begin{example}
  ...
  \begin{explanation}
    ...
    Hooke's law.  So we must have $F(x) = 4x$.  So the work done by
    moving the spring from $x$ to $x+\d x$ is
    \[
    \d W = \answer[given]{4x} \d x.
    \]
    We need to accumulate these infinitesimal forces from $x=0$ to
    $x=5$.
    \begin{align*}
