...
\begin{definition}
  The \dfn{orthogonal projection}\index{projection} in the direction of a vector $\vec{w}$ of a
...
\begin{theorem}
  ...
  %%   \[
  %%   \proj_\vec{w}(\vec{v}) = c \vec{w}
  %%   \]
  %%   for some real number $c$.  We also know that
  %%   \[
...
\begin{definition}
  The \dfn{scalar component}\index{component} in the direction of
...
\begin{example}
  ...
  \begin{explanation}
    ...
    compute $\proj_\vec{g}(\vec r)$:
    \begin{align*}
      \proj_\vec{g}(\vec{r}) &= \left(\frac{\vec{g}\cdot\vec{r}}{\vec{r}\cdot\vec{r}}\right)\vec r\\
	...
	\draw [penColor4,very thick,->] (13,-3.15) -- (11.125,-3.625)node [shift={(10pt,-5pt)} ,penColor4,pos=0] {\scriptsize $\proj_{\vec g}(\vec r)$};
      \end{scope}
    \end{tikzpicture}
    \end{image}
\end{explanation}
