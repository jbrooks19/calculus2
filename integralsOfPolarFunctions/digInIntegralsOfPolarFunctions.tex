...
\begin{prompt}
  \begin{multipleChoice}
    ...
    \choice{$\pi/4 = \pi/2$}
    \choice{a complete curve is drawn when $\theta$ runs from $0$ to $\pi$}
  \end{multipleChoice}
  \begin{feedback}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{image}
      \begin{tikzpicture}
        \begin{polaraxis}[
          ...
          %\addplot+[draw=none, fill=fill1,mark=none,domain=45:225,samples=100,smooth] {1+cos(x)}\closedcycle;
          %\addplot+[draw=none, fill=fill2,mark=none,domain=-135:45,samples=100,smooth] {1+sin(x)}\closedcycle;
    ...
    \[
    \int_{\pi/4}^{5\pi/4} (1+\cos(\theta))^2 \d \theta \qquad\text{or}\qquad\int_{-3\pi/4}^{\pi/4} (1+\cos(\theta))^2 \d \theta
    \]
    Evaluating either integral tells us the area is
    $\answer[given]{\frac{3\pi}{2}-2\sqrt{2}}$.
  \end{explanation}
\end{example}
