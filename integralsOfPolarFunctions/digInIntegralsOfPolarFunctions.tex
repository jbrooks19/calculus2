...
%% integral.\index{polar!functions!area}
...
%% Consider Figure \ref{fig:polararea} (a) where a region defined by
%% $r=f(\theta)$ on $[\alpha,\beta]$ is given. (Note how the ``sides'' of
%% the region are the lines $\theta=\alpha$ and $\theta=\beta$, whereas
%% in rectangular coordinates the ``sides'' of regions were often the
%% vertical lines $x=a$ and $x=b$.)  \mnote{.8}{\textbf{Note:} Recall
%%   that the area of a sector of a circle with radius $r$ subtended by
%%   an angle $\theta$ is $A = \frac12\theta r^2$.
