...
\begin{question}
  ...
  \begin{question}
    ...
    \begin{prompt}-
      \[
      \answer{-4} + \answer{-9}(x-1) + \answer{-3}(x-1)^2 + (x-1)^3
      \]
    \end{prompt}
  \end{question}
\end{question}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \end{array}
    \]
    Now seeing that a repeating pattern has emerged, we see that the
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \end{array}
    \]
    Now seeing that a pattern has emerged, we see that the
    Taylor series for $\ln(x)$ is:
    \[
    (x-1) - \frac{(x-1)^2}{2} + \frac{(x-1)^3}{3} - \frac{(x-1)^4}{4} + \cdots = \sum_{n=1}^\infty (-1)^{n+1}\frac{(x-1)^n}{n}
    \]
  \end{explanation}
\end{example}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{align*}
    \frac{1}{1-(-x^2)} &= 1 + (-x^2)+ (-x^2)^2 + (-x^2)^3 + \cdots &|(-x^2)|< 1\\
...
\begin{example}
	...
	It turns out that $f$ is infinitely differentiable everywhere,
        ...
        1 + 0 + 0+ \cdots
        \]
        The derivatives of $f$ ``goes to $0$'' as $x$ goes to zero
...
\begin{example}
	...
	\begin{explanation}
	  To start, note that using the ratio test this power series
          ...
          We know $f^{(n+1)}(b) = e^b$, since all the derivatives of
          $e^x$ are just $e^x$
	  Thus
	  \[
	  R_n(x) = \frac{e^b}{(n+1)!}x^{n+1}
	  \]
