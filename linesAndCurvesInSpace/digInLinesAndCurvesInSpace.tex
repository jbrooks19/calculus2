...
line $\l$. How do we find this distance? Well, we could:
\begin{enumerate}
  \item Recalling that the magnitude of a vector $|\vec{w}|
    =\sqrt{\vec{w}\dotp\vec{w}}$ we could attempt to minimize this
    function
    \[
    \mathrm{distance}(t)^2= \l(t) \cdot \vec{p}
    \]
    using the derivative. The square-root of the minimum value will be
    the distance.
  \item We could project the vector $\vec{p}-\vec{q}$ along $\vec{v}$
\end{enumerate}
BADBAD
...
\[
\vec{f}(t) = \vec{p}+r\cdot \cos(t)\cdot \vec{u} + r \cdot \sin(t)\cdot\vec{v}
\]
gives a circle of radius $r$, centered at the tip of $\vec{p}$, lying
in the plane containing $\vec{u}$ and $\vec{v}$. Moreover, to produce
an ellipse, we write:
\[
\vec{g}(t) = \vec{p}+a\cdot \cos(t)\cdot \vec{u} + b \cdot \sin(t)\cdot\vec{v}
\]
