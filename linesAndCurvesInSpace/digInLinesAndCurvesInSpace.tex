...
others, or in the opposite direction, or even at uneaven rates!

Often, for a given line, we will already know that the line passes
through a point $\vec{p}$, and points in a direction $\vec{v}$.  Then
...
the direction $\vec{v} = \vec{q} - \vec{p}$, and passes through
$\vec{P}$.  So we can parameterize it as
\[
\boldsymbol{\l}(t) = \vec{p}+t(\vec{q} - \vec{p}).
\]
...
Sometimes lines lie on suprising surfaces:

\begin{question}
  Consider the surface determinded by all $x$, $y$ and $z$ such that:
  \[
  x^2+y^2=z^2+1
  \]
