...
The \dfn{standard form}\index{standard form!for a circle} for a circle
...
favor and play around with Lissajous figures for differening values of
$\delta$, $a$ and $b$.
...
\item Use a trigonometric idenitity.
\end{itemize}
In each case the process that we are using is called
\dfn{elimination of a parameter}.

We'll give several examples of how one actually \textit{elimiates a
  parameter}.
...
\begin{example}
  ...
  \end{align*}
    ...
    \begin{explanation}
      ...
      \begin{align*}
        y(t) &= e^{3t}\\
        y(t) &= \left(e^{t}\right)^3.
      \end{align*}
      Now we see that
      \[
      y = \left(\frac{x}{-5}\right)^3.
      \]
  \end{explanation}
\end{example}
