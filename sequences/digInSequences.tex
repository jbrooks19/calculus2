...
\begin{example}
  ...
  \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=1.5cm]
    \node (a1) {$10,$};
    \node (a2) [right of=a1] {$14,$};
    \node (a3) [right of=a2] {$18,$};
    \node (a4) [right of=a3] {$22,$};
    \node (a5) [right of=a4] {$\ldots$};

    \path[->] (a1) edge [bend left=20] node {$+4$} (a2);
    \path[->] (a2) edge [bend left=20] node {$+4$} (a3);
    \path[->] (a3) edge [bend left=20] node {$+4$} (a4);
    \path[->] (a4) edge [bend left=20] node {$+4$} (a5);
  \end{tikzpicture}
  \end{image}
...
\begin{example}
  ...
  \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=1.5cm]
    ...
    \path[->] (a1) edge [bend left=20] node {$-2$} (a2);
    \path[->] (a2) edge [bend left=20] node {$-2$} (a3);
    \path[->] (a3) edge [bend left=20] node {$-2$} (a4);
    \path[->] (a4) edge [bend left=20] node {$-2$} (a5);
  \end{tikzpicture}
  \end{image}
...
\begin{question}
  ...
  \begin{selectAll}
    \choice{$1,1,1,1,1,\dots$}
    \choice{$6,11,16,21,26\dots$}
    \choice{$a_n = -3n + 5$}
    \choice{$a_n = \frac{4x^2+x-14}{x+2}$}
